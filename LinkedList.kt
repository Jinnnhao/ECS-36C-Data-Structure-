...
class LinkedList<T> {
    ...
    fun mapInPlace(operator: (T) -> T): LinkedList<T> {
        for (cell in this.cellIterator()) {
            cell.data = operator(cell.data as T)
        }
        return this
    }
    ...
    fun insertionSortWith(comp: Comparator<in T>): LinkedList<T> {
        ...
        while (atCell != null) {
            ...
            atCell = atCell.next
            current.next = null
            if (comp.compare(current.data as T, headCell!!.data as T) < 0) {
                current.next = headCell
                headCell = current
            } else {
                var insertAt = headCell
                while (insertAt!!.next != null) {
                    if (comp.compare(current.data as T, insertAt.next!!.data as T) < 0) {
    ...
    fun split(start: LinkedListCell<T>) : Pair<LinkedListCell<T>?,LinkedListCell<T>?> {
        ...
        var start_of_second = end_of_first.next
    ...
    fun mergeSortWith(comp: Comparator<in T>) : LinkedList<T> {
        fun mergeSortInternal(cells: LinkedListCell<T>) : LinkedListCell<T> {
            ...
            // Now set the head of result and use at to iterate through the list to link up each node
            // if it is not reverse, we do firstHalt compareTo second
            if(comp.compare(secondHalf.data, firstHalf.data) == -1) // if second < first
            {

                result = secondHalf
                secondHalf = secondHalf.next
            }
            // if it is reverse and first < second
            else if (comp.compare(secondHalf.data, firstHalf.data) == -1)
            {
                result = secondHalf
                secondHalf = secondHalf.next
            }
            ...
            // preserve the result as it indicating the head of the sort linkedlist
            at = result
            at.next = null
            while (firstHalf != null)
            {
                // if secondHalf = null, then link to the firstHalf and return, since the firstHalf is already sorted?
                if(secondHalf == null)
                {
                    at?.next = firstHalf
                    return result
                }
                // Not reverse and second < first
                else if (comp.compare(secondHalf.data, firstHalf.data) == -1)
                {
                    at?.next = secondHalf
                    secondHalf = secondHalf.next
